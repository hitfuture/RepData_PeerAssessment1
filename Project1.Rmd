...
**Total Number of missing values:** `r prettyNum(nrow(activities%>%filter(is.na(steps))),big.mark=",")`
...
* replace NA in the activities dataframe with the mean value.   
...
for(i in 1:nrow(activities2)){
        ...
        if(is.na(activity$steps)){
                ...
                activities2[i,]$steps<-mean.val
        }
        
}

       
activities.by.day2<-activities2%>%group_by(date)%>%summarize(total.steps.per.day=sum(steps))
mean.val<-mean(activities.by.day2$total.steps.per.day[!is.na(activities.by.day2$total.steps.per.day)])
median.val<-median(activities.by.day2$total.steps.per.day[!is.na(activities.by.day2$total.steps.per.day)])

#kable(activities2[which(is.na(activities$steps)),])
